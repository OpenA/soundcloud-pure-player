...
(function(){
	...
	SoundCloudAPI.prototype = {
		...
		getTrackInfo: function(url, callback) {
			...
			var resolve = (secureDocument ? 'https' : 'http') + '://api.soundcloud.com/resolve?url=',
			...
			ajax(apiUrl, function(json) {
				try {
					var trackObj = JSON.parse(json);
				} catch(e) {
		...
		removeEventListener: function(eventType, eventAction) {
	...
	function ajax(url, callback, debug) {
		if (!url || typeof callback !== 'function')
			return;
		var xhr = new ('onload' in XMLHttpRequest.prototype ? XMLHttpRequest : XDomainRequest)();
	...
	window.soundcloud = new SoundCloudAPI;
})();
